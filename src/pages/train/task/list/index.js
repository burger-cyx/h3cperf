...
import { TaskListContent, TaskListTitle } from "../../../../components/task/list";
import { getMenuByName } from "../../../../config/header";
import { useSearchParams } from "react-router-dom";
import { trainTaskExec, trainTaskDelete, trainTaskList } from "../../../../api";

const TrainTaskList = () => {
  const menuList = getMenuByName("训练任务列表");
  const title = {
    name: menuList.name,
  ...
  const runTaskAndRefresh = (id_list) => {
    trainTaskExec(id_list).then(()=>{
      // 添加后刷新 tableData
      fetchTaskList(tplId, 1)
    })
  }
  const deleteTaskAndRefresh = (id_list) => {
    trainTaskDelete(id_list).then(()=>{
      // 删除后刷新 tableData
      fetchTaskList(tplId, 1)
    })
  }
